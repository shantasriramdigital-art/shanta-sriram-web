...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'send-link') {
        ...
        <html>
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #F8F4EF; padding: 20px;">
            <div style="max-width: 560px; margin: 0 auto; background: white; border: 1px solid #E8ECF0; border-radius: 6px; padding: 28px;">
              ...
              <p style="color: #6B6B6B; font-size: 14px; margin: 0 0 24px 0;">Click the button below to reset your password:</p>
              <div style="text-align: center; margin: 24px 0;">
                <a href="${siteUrl}/reset-password?tab=direct&email=${encodeURIComponent(body.email)}" style="display: inline-block; background: #CD0E12; color: white; padding: 14px 28px; border-radius: 4px; text-decoration: none; font-weight: 500;">Reset Password</a>
              </div>
              <p style="color: #6B6B6B; font-size: 12px; margin: 0;">This link is valid for 24 hours.</p>
            </div>
          </body>
        </html>
      `,
    ...
    return NextResponse.json({ ok: true, message: 'Reset instructions sent to email' })
  }
